...
public class RestResourceAdapter {
	...
	private Resource2HttpContext resourceContext;
	...
	private void initResourceMapping(BundleContext applicationContext) {
		resourceContext = new Resource2HttpContext();
	...
	private synchronized void reloadApplication() {
		...
		properties.put(FreemarkerMvcFeature.TEMPLATES_BASE_PATH, "/templates");
		properties.put(FreemarkerMvcFeature.TEMPLATE_OBJECT_FACTORY, createFreemarkerConfiguration());
		...
		// for (WebResource resource2 : resourceInstances) {
		// Bundle bundle = FrameworkUtil.getBundle(resource.getClass());
		// String path = "/" +
		// trim(resource.getClass().getAnnotation(Path.class).value(), '/');
